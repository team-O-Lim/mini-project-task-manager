...
use `mini-project-task-maneger-db`;
...
CREATE TABLE IF NOT EXISTS `projects` (
  ...
  description VARCHAR(255) NOT NULL, -- // (프로젝트 설명: comment , description) **
...
CREATE TABLE tasks (
	...
	CONSTRAINT fk_task_assignee FOREIGN KEY (assigned_user) REFERENCES users(id),
	INDEX idx_task_project_status (project_id, status),
	INDEX idx_task_assignee_due (assigned_user, due_date)
) ENGINE=InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci
  COMMENT = '직무'; -- 규격 통일
 ...
 CREATE TABLE task_assigned_users (
	...
	CONSTRAINT fk_auth_assigned FOREIGN KEY (assigned_id) REFERENCES tasks(assigned_user) ON DELETE CASCADE,
	CONSTRAINT fk_auth_users  FOREIGN KEY (user_id)  REFERENCES users(id) ON DELETE CASCADE,
    
    CONSTRAINT uk_task_users_assigned UNIQUE (assigned_id, user_id)
) ENGINE=InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci
  COMMENT = '직무 담당자 지정';
  
  SELECT * FROM task_assigned_users;
  
  drop table task_assigned_users;
